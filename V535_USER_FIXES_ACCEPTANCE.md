...
Release-identitás: a végső candidate csak akkor tekinthető v5.3.5-nek, ha a kanonikus `cpap.version.APP_VERSION` pontosan `5.3.5`, a `RELEASE_NOTES.md` első kiadási szekciója `# SleepMate 5.3.5`, és az exact-SHA VERIFIED ZIP/MSI/runtime metadata ugyanezt a verziót és ugyanazt a commit SHA-t igazolja. A `v534` nevű regressziótesztek, a `frontend-v534.js` és a `UI_VERSION = 5.3.4` a bizonyított frontend-generáció történeti azonosítói maradhatnak, de az aktív PWA shell/API cache generációnak már `sleepmate-shell-v5.3.5…` / `sleepmate-api-v5.3.5…` néven kell futnia, hogy az 5.3.5 frissítés biztosan új cache-generációt kapjon. A régebbi v5.2/v5.3.2/v5.3.4 regressziótesztek csak történeti kompatibilitási markereket tarthatnak meg; az aktuális APP_VERSION- és aktív cache-elvárásuknak a 5.3.5 release-identitást kell követnie.