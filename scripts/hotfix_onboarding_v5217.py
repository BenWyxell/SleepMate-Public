...
def replace_required(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise SystemExit(f"Missing {label} patch target: {old[:180]!r}")
    return text.replace(old, new)


js_path = Path("web/first-run.js")
...
js = replace_required(
    ...
    "broken onboarding brand image",
)
js = replace_required(
    js,
    "A program már telepítve van. Itt azokat a dolgokat állítjuk be, amelyek gépenként vagy felhasználónként eltérnek: terápiás adatok, SleepSync, távoli elérés, PWA, értesítések, backup és opcionális AI.",
    "A program már telepítve van. Itt azokat a dolgokat állítjuk be, amelyek gépenként vagy felhasználónként eltérnek: terápiás adatok, SleepSync, távoli elérés, backup és opcionális AI.",
    "wizard intro copy",
)
js = replace_required(
    js,
...
js = replace_required(
    ...
    "wizard step scroll reset",
)
js = replace_required(
    js,
    "Újra végigvezet az adatforrás, SleepSync, távoli elérés, PWA, backup és AI alapbeállításain.",
    "Újra végigvezet az adatforrás, SleepSync, távoli elérés, backup és AI alapbeállításain.",
    "reopen copy",
)
js_path.write_text(js, encoding="utf-8")

css_path = Path("web/first-run.css")
css = css_path.read_text(encoding="utf-8")
css_pairs = [
    (
        ".fr-shell{width:min(980px,100%);max-height:min(820px,calc(100vh - 52px));display:grid;grid-template-rows:auto 1fr auto;",
        ".fr-shell{width:min(980px,100%);height:min(820px,calc(100vh - 52px));height:min(820px,calc(100dvh - 52px));max-height:calc(100vh - 52px);max-height:calc(100dvh - 52px);min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;",
        "wizard shell sizing",
    ),
    (".fr-top{display:flex;", ".fr-top{position:relative;z-index:2;display:flex;", "header layer"),
    (
        ".fr-brand{display:flex;align-items:center;gap:13px;min-width:215px}.fr-logo{width:50px;height:50px;border-radius:16px;object-fit:contain;filter:drop-shadow(0 8px 18px rgba(0,0,0,.25))}",
        ".fr-brand{display:flex;align-items:center;min-width:215px}.fr-logo{display:none}",
        "brand layout",
    ),
    (
        ".fr-body{overflow:auto;padding:34px 38px 28px;scrollbar-width:thin}",
        ".fr-body{min-width:0;min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-gutter:stable;padding:34px 38px 28px;-webkit-overflow-scrolling:touch}",
        "global wizard scroll body",
    ),
    (
        ".fr-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}",
        ".fr-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.fr-grid.single{grid-template-columns:1fr}",
        "single-column extras grid",
    ),
    (".fr-footer{display:flex;", ".fr-footer{position:relative;z-index:2;display:flex;", "footer layer"),
    (
        "#sleepmateFirstRun.fr-hidden{display:none}",
        "#sleepmateFirstRun.fr-hidden{display:none}html.fr-open,html.fr-open body{overflow:hidden}",
        "background scroll lock",
    ),
    (
        "@media(max-width:760px){#sleepmateFirstRun{padding:0}.fr-shell{height:100%;max-height:none;border-radius:0;border:0}",
        "@media(max-width:760px){#sleepmateFirstRun{padding:0}.fr-shell{height:100vh;height:100dvh;max-height:none;min-height:0;border-radius:0;border:0}",
        "mobile viewport",
    ),
]
for old, new, label in css_pairs:
    css = replace_required(css, old, new, label)
compact = "@media(max-height:700px) and (min-width:761px){#sleepmateFirstRun{padding:12px}.fr-shell{height:calc(100vh - 24px);height:calc(100dvh - 24px);max-height:calc(100vh - 24px);max-height:calc(100dvh - 24px)}.fr-top{padding:14px 22px}.fr-body{padding:24px 30px 20px}.fr-footer{padding:12px 20px}}\n"
marker = "@media(max-width:760px)"
if marker not in css:
    raise SystemExit("Missing responsive CSS marker")
css = css.replace(marker, compact + marker, 1)
css_path.write_text(css, encoding="utf-8")

hydration_path = Path("web/sleepsync-hydration-v529.js")
hydration = hydration_path.read_text(encoding="utf-8")
hydration = replace_required(
    hydration,
    "script.src='/first-run.js?v=1';",
    "script.src='/first-run.js?v=2';",
    "first-run JS cache-buster",
)
hydration_path.write_text(hydration, encoding="utf-8")

test_path = Path("tests/test_first_run_wizard_v5217_hotfix.py")
...
print("Onboarding hotfix patch applied.")
