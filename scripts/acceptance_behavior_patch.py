...
# Keep the strict exact-value assertion, and align its diagnostic marker with the
# matrix contract (this does not relax the assertion).
rep(
...
# Dashboard aggregate must remain visible with exactly one matched night, not only
# with a multi-night data set.
rep(
"""        page.evaluate(
            """() => {
              const f=window.__smAcceptanceO2;
...
# be present on the x axis. Restore the daily page afterwards.
rep(
...
# Landscape must preserve the same aligned graph geometry as portrait, not only
# keep the toolbar inside the viewport.
rep(
