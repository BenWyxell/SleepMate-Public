...
def _candidate_ports() -> list[int]:
    ...
    try: preferred = int(cfg.get("port", 8895) or 8895)
    except Exception: preferred = 8895
    if str(cfg.get("port_mode") or "auto").lower() == "fixed": return [preferred]
...
def _sleepmate_port() -> int | None:
    for port in _candidate_ports():
        try:
            ...
            with urllib.request.urlopen(req, timeout=0.22) as r: obj = json.loads(r.read().decode("utf-8"))
            if obj.get("app") == "SleepMate": return int(port)
...
def _recover_stuck_tray() -> bool:
    if os.name != "nt" or not _tray_mutex_exists(): return False
    if _sleepmate_port() is not None: return False
    try:
        private = state_root(app_root()) / "private"; pid_file = private / "tray.pid"; heartbeat = private / "tray_heartbeat.json"
        pid = int(pid_file.read_text(encoding="ascii").strip()); hb = json.loads(heartbeat.read_text(encoding="utf-8"))
        if int(hb.get("pid") or 0) != pid or pid <= 0: return False
        if time.time() - heartbeat.stat().st_mtime <= TRAY_HEARTBEAT_FRESH_SECONDS: return False
    except Exception: return False
    end = time.time() + 3.5
    while time.time() < end:
        if _sleepmate_port() is not None: return False
        ...
        time.sleep(0.35)
    try:
        subprocess.run(["taskkill", "/PID", str(pid), "/T", "/F"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=CREATE_NO_WINDOW, timeout=10, check=False)
        for _ in range(30):
            if not _tray_mutex_exists(): return True
            time.sleep(0.1)
...
def main() -> int:
    ...
    if "--migrate-from" in sys.argv:
        ...
        if not source:
            print("Missing path after --migrate-from", file=sys.stderr); return 2
        result = migrate_from_path(Path(source), app_root()); print(json.dumps(result, ensure_ascii=False))
        if "--migrate-only" in sys.argv: return 0 if not result.get("errors") else 3
        i = sys.argv.index("--migrate-from"); del sys.argv[i:i + 2]
