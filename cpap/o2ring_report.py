...
def _oximetry_section(self: rp.SleepMateReport) -> None:
    ...
    self._title(
        "Oximetria és pulzus",
        "Wellue / Viatom O2Ring – kizárólag a PAP-terápiás szakaszokkal időben átfedő mérések.",
    )

    def vals(key):
        return [float(r[key]) for r in rows if r.get(key) is not None]

    spo2_avg = vals("spo2_average")
    spo2_min = vals("spo2_minimum")
    t90 = vals("t90_seconds")
    hr_avg = vals("heart_rate_average")
    odi3 = vals("odi3")
    odi4 = vals("odi4")
    ...
    self.story.append(self._line_chart(
        [[r.get("spo2_average") for r in rows], [r.get("spo2_minimum") for r in rows]],
        labels,
        [self.chart_primary, self.chart_secondary],
        y_min=70,
        y_max=100,
    ))
    self.story.append(Spacer(1, 4 * mm))
    self._chart_title("Pulzus trend", "Átlagpulzus a CPAP-időszakokra illesztett O2Ring mintákból.")
    self.story.append(self._line_chart(
        [[r.get("heart_rate_average") for r in rows]], labels, [self.chart_primary], y_min=30,
    ))
    self.story.append(Spacer(1, 4 * mm))
    self._chart_title("T90 trend", "A 90% alatti oxigénszint időtartama percben.")
    self.story.append(self._line_chart(
        [[float(r.get("t90_seconds") or 0) / 60.0 for r in rows]],
        labels,
        [self.chart_secondary],
        y_min=0,
    ))
    self.story.append(Spacer(1, 4 * mm))
...
def _build_with_oximetry(self: rp.SleepMateReport, out_path: Path) -> Path:
    ...
    if self.config.get("include_patient"):
        self._patient_page()
    if self.section_enabled("summary"):
        self._summary()
    if self.section_enabled("usage"):
        self._usage()
    if self.section_enabled("events"):
        self._events()
    if self.section_enabled("pressure_leak"):
        self._pressure_leak()
    if self.section_enabled("comparison") and self.compare_previous:
        self._comparison()
    if self.section_enabled("oximetry"):
        self._oximetry()
    if self.section_enabled("timeline"):
        self._timeline_section()
    if self.section_enabled("calendar"):
        self._calendar()
    if self.section_enabled("assessments"):
        self._assessments()
    if self.section_enabled("equipment"):
        self._equipment()
    if self.section_enabled("diagnosis"):
        self._diagnosis()
    if self.section_enabled("data_quality"):
        self._data_quality()
    if self.section_enabled("daily_table"):
        self.story.append(CondPageBreak(48 * mm))
        self._daily_table()
    if self.section_enabled("glossary"):
        self._glossary()
