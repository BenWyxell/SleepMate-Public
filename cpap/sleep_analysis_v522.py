...
def _range_from_period(rows: list[dict[str, Any]], period_raw: str):
    """Resolve display ranges from the latest available sleep day."""
    if not rows:
    ...
    if period_raw == "day":
        return latest, latest, "Legutóbbi alvásnap"
...
def analyze(self: sa.SleepAnalysisService, dataset, period: str = "all") -> dict[str, Any]:
    ...
    period_raw = str(period or "all").lower().strip()
    start, end, label = _range_from_period(all_rows, period_raw)

    rows = all_rows
    if start is not None and end is not None:
        rows = [row for row in rows if start <= datetime.fromisoformat(row["date"]).date() <= end]
