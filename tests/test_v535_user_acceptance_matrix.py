...
RELEASE_NOTES = read('release-notes/v5.3.27.md')
...
def test_release_identity_is_v535():
    assert APP_VERSION == '5.3.27'
    assert RELEASE_NOTES.startswith('# SleepMate 5.3.27')


def test_release_cache_generation_is_v535_while_frontend_generation_remains_v534():
    for worker in (SW, SW_BASE):
        assert "const SHELL_CACHE='sleepmate-shell-v5.3.27';" in worker
        assert "const API_CACHE='sleepmate-api-v5.3.27';" in worker
        assert "const UI_VERSION='5.3.4';" in worker
        assert '/frontend-v534.js?v=5.3.27' in worker
