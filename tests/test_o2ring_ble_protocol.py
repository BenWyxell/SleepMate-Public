...
def test_live_sensor_payload_matches_reference_layout():
    ...
    # SpO2 98, HR 66, battery 97, motion 146, signal strength 10, finger present.
    payload = bytes.fromhex("624200000000006100920a0100")
    ...
    # Temporary API compatibility only; it must not be presented as medical PI.
...
def test_discovery_accepts_known_ring_name_without_service_uuid():
    assert O2RingBLEManager._looks_like_supported_device("O2Ring 1234", set()) is True
    assert O2RingBLEManager._looks_like_supported_device("CheckO2", set()) is True


def test_discovery_accepts_service_uuid_but_not_generic_o2_name():
    assert O2RingBLEManager._looks_like_supported_device("Unknown", {SERVICE_UUID}) is True
    assert O2RingBLEManager._looks_like_supported_device("My O2 Speaker", set()) is False
