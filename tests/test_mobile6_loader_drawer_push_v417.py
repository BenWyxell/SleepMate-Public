...
def test_pwa_navigation_does_not_create_history_for_edge_back_gesture():
    assert "if(standalonePwa())" in JS_COMPACT
    assert "history.replaceState({sleepmate:true},'',next)" in JS_COMPACT
    assert 'e.preventDefault()' in JS_COMPACT
    assert "{passive:false}" in JS_COMPACT
