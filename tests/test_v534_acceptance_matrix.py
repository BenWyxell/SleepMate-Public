...
def test_acceptance_p0_single_frontend_owner_and_stale_pwa_recovery():
    ...
    for worker in (sw, base):
        assert "sleepmate-shell-v5.3.25" in worker
        assert "sleepmate-api-v5.3.25" in worker
        assert "/o2ring.js" in worker
        assert "/frontend-v534.js?v=5.3.25" in worker
        assert "/o2ring-v534.css?v=5.3.25" in worker
        assert "/o2ring-v532.js" not in worker
        assert "/frontend-v533.js" not in worker
        assert "RELEASE_VERSION='5.3.25'" in worker
