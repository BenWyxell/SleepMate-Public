...
function updateAIAnalysisLocks(){const rows=state.ai.status?.current_dataset_analyses||{},features=aiFeaturePreferences();$$('[data-ai-analysis]').forEach(btn=>{const key=currentAIAnalysisKey(btn.dataset.aiAnalysis),lock=features.assistantsVisible?rows[key]:null;btn.classList.toggle('locked',!!lock);btn.dataset.savedAnalysisId=lock?.analysis_id||'';btn.textContent=lock?'Mentett megnyitása':!features.assistantsVisible&&features.promptingEnabled?'Prompt elkészítése':'Kiértékelés';btn.title=lock?'Ehhez az adatverzióhoz már van mentett kiértékelés. Kattints a visszanézéshez.':''})}
...
async function startAIAnalysisWithFeatures(type,button){if(!state.days.length){showError(new Error('Még nincs kiértékelhető terápiás adat.'));return}const features=aiFeaturePreferences();if(state.ai.provider==='gemini'&&!features.lunaVisible&&features.miloVisible)selectAIProvider('groq');else if(state.ai.provider==='groq'&&!features.miloVisible&&features.lunaVisible)selectAIProvider('gemini');let selection;try{selection=aiSelection(type)}catch(e){showError(e);return}const providerVisible=selection.provider==='groq'?features.miloVisible:features.lunaVisible,promptTask=features.promptingEnabled?requestAIPrompt(selection):null;if(providerVisible){if(promptTask)promptTask.catch(showError);return startAIAnalysis(type,button)}if(promptTask){button.disabled=true;try{await promptTask}catch(e){showError(e)}finally{button.disabled=false}}}
