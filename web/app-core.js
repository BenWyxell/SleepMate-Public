...
async function init(){
  ...
  try{
    ...
    await Promise.all([loadDays(),loadConfig()]);
    route();
...
function bindUi(){
  ...
  $$('[data-ai-analysis]').forEach(b=>b.onclick=()=>startAIAnalysis(b.dataset.aiAnalysis,b));
...
async function loadDayRows(force=false){
  if(state.dayRows.length&&!force)return state.dayRows;
  const d=await api('/api/day-table');state.dayRows=d.rows;applyOximetryVisibility();return state.dayRows;
}
...
async function createDailyShareCard(){const s=state.summary;if(!s)throw new Error('Nincs megosztható napi adat.');const c=document.createElement('canvas');c.width=1080;c.height=1350;const x=c.getContext('2d');x.fillStyle='#08111f';x.fillRect(0,0,c.width,c.height);x.fillStyle='#edf4fb';x.font='700 70px system-ui';x.fillText('SleepMate',70,115);x.fillStyle='#95aabe';x.font='38px system-ui';x.fillText(formatDayCode(s.day),70,175);x.fillStyle='#152434';x.fillRect(60,230,960,900);const stats=[['Használati idő',formatUsageShort(s.usage),''],['AHI',Number(s.ahi||0).toFixed(2),'esemény/óra'],['Szivárgás 95%',String($('#dailyLeakP95')?.textContent||'–'),'L/perc'],['Nyomás 95%',String($('#dailyPressureP95')?.textContent||'–'),'cmH₂O'],['Események',String($('#eventCount')?.textContent||'–'),`OA ${s.counts?.OA||0} • CA ${s.counts?.CA||0} • H ${s.counts?.H||0} • RERA ${s.counts?.RERA||0}`]];stats.forEach((a,i)=>{const y=315+i*160;x.fillStyle='#91a8bd';x.font='32px system-ui';x.fillText(a[0],105,y);x.fillStyle='#f4f8fb';x.font='700 64px system-ui';x.fillText(a[1],105,y+72);if(a[2]){x.fillStyle='#91a8bd';x.font='27px system-ui';x.fillText(a[2],430,y+65)}});x.fillStyle='#91a8bd';x.font='26px system-ui';x.fillText('PAP-terápiás összefoglaló • SleepMate',70,1240);return await new Promise((res,rej)=>c.toBlob(b=>b?res(b):rej(new Error('A megosztási kép nem készíthető el.')),'image/png',.94))}
async function shareCurrentDay(){if(!state.summary){showError(new Error('Nincs megosztható napi adat.'));return}const s=state.summary,text=`SleepMate • ${formatDayCode(s.day)}\nHasználat: ${formatUsageShort(s.usage)}\nAHI: ${Number(s.ahi||0).toFixed(2)} /óra\nSzivárgás P95: ${$('#dailyLeakP95')?.textContent||'–'} L/perc\nNyomás P95: ${$('#dailyPressureP95')?.textContent||'–'} cmH₂O\nEsemények: OA ${s.counts?.OA||0}, CA ${s.counts?.CA||0}, H ${s.counts?.H||0}, RERA ${s.counts?.RERA||0}`;try{if(navigator.share){const blob=await createDailyShareCard(),file=new File([blob],`SleepMate_${s.day}.png`,{type:'image/png'});if(!navigator.canShare||navigator.canShare({files:[file]})){await navigator.share({title:`SleepMate – ${formatDayCode(s.day)}`,text,files:[file]});return}await navigator.share({title:`SleepMate – ${formatDayCode(s.day)}`,text});return}await navigator.clipboard.writeText(text);addLog('INFO','Napi összefoglaló a vágólapra másolva.')}catch(e){if(e?.name!=='AbortError')showError(e)}}
...
function applyOximetryVisibility(summary=state.summary){
  ...
  const spo2Value=summary?.oximetry?.spo2_median,hrValue=summary?.oximetry?.pulse_median;
  if($('#spo2')){$('#spo2').textContent=spo2Value!=null?`${spo2Value}%`:'Nincs adat';$('#spo2').classList.toggle('no-data',spo2Value==null)}
  if($('#hr')){$('#hr').textContent=hrValue!=null?`${hrValue}`:'Nincs adat';$('#hr').classList.toggle('no-data',hrValue==null)}
}
...
function chooseMobileAIAnalysis(type){
  ...
  closeAIAnalysisSheet();const btn=document.querySelector(`[data-ai-analysis="${type}"]`);if(btn)startAIAnalysis(type,btn);
}
function runMobileAIMonth(){const mobile=$('#aiMobileMonthSelect'),desktop=$('#aiMonthSelect');if(desktop&&mobile)desktop.value=mobile.value;updateAIAnalysisLocks();closeAIAnalysisSheet();const btn=document.querySelector('[data-ai-analysis="month"]');if(btn)startAIAnalysis('month',btn)}
function runMobileAIComparison(){for(const [mobile,desktop] of [['aiMobileCompareAStart','aiCompareAStart'],['aiMobileCompareAEnd','aiCompareAEnd'],['aiMobileCompareBStart','aiCompareBStart'],['aiMobileCompareBEnd','aiCompareBEnd']])if($(mobile)&&$(desktop))$(desktop).value=$(mobile).value;updateAIAnalysisLocks();closeAIAnalysisSheet();const btn=document.querySelector('[data-ai-analysis="comparison"]');if(btn)startAIAnalysis('comparison',btn)}
...
function updateAIAnalysisLocks(){const rows=state.ai.status?.current_dataset_analyses||{};$$('[data-ai-analysis]').forEach(btn=>{const key=currentAIAnalysisKey(btn.dataset.aiAnalysis),lock=rows[key];btn.classList.toggle('locked',!!lock);btn.dataset.savedAnalysisId=lock?.analysis_id||'';btn.textContent=lock?'Mentett megnyitása':'Kiértékelés';btn.title=lock?'Ehhez az adatverzióhoz már van mentett kiértékelés. Kattints a visszanézéshez.':''})}
...
async function installAvailableUpdate(){
  confirmAction('A SleepMate teljes backupot és rollback-pontot készít, majd újraindul az új verzióban. Ha az új build nem indul megfelelően, a külső frissítő automatikusan visszaáll az előző verzióra. Folytatod?',async()=>{
    try{clearError();const r=await apiWrite('/api/update/install','POST',{});setProgressBox('update',{progress:1,phase:'Indítás',message:'Biztonságos frissítés előkészítése…'});let expected='';try{const j=await pollJob(r.job,'update');expected=j.result?.target_version||'';setProgressBox('update',{status:'done',progress:100,phase:'Újraindítás',message:`SleepMate ${expected||'új verzió'} indul…`})}catch(e){/* az újraindítás közben megszakadó HTTP kapcsolat itt normális */}const ok=await waitForSleepMateRestart(expected);if(!ok)showError(new Error('A SleepMate újraindítása a várt időn belül nem igazolható. Nézd meg az update_worker naplót vagy indítsd el a programot; sikertelen boot esetén a rollback automatikus.'))}catch(e){showError(e);setProgressBox('update',{status:'error',phase:'Frissítési hiba',message:e.message})}
  },'SleepMate frissítése')
}
...
async function loadDashboard(day){
  ...
  try{
    const s=await api(`/api/day/${day}`);const extras=await Promise.allSettled([api(`/api/day/${day}/stats`),api('/api/patient'),api('/api/patient/therapy?period=30')]);const st=extras[0].status==='fulfilled'?extras[0].value:{rows:[],apnea_duration:'–'},patient=extras[1].status==='fulfilled'?extras[1].value:(state.patient||{}),therapy30=extras[2].status==='fulfilled'?extras[2].value:{};extras.forEach((r,i)=>{if(r.status==='rejected')addLog('WARN',`Napi kiegészítő adat ${i+1}: ${r.reason?.message||r.reason}`)});state.currentDay=day;state.patient=patient;state.summary=s;state.full=fullBounds(s);state.view=[...state.full];state.hoverTime=null;state.overviewSignals.clear();state.stackSignals.clear();state.mainSignal=null;state.selectedSignal=state.selectedSignal||'flow';state.dayToken++;state.mainToken++;state.stackToken++;
