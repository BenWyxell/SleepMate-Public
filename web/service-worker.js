...
const API_CACHE='sleepmate-api-v5.3.3-recovery';
const SHELL=[
  '/',
  '/index.html',
  '/style.css?v=5.3.3',
  '/app.js?v=5.3.3',
  '/app-engine119.js?v=130',
  '/app-core.js?v=5.0.8',
  '/sleepsync.css?v=engine-2',
  '/sleepsync-base.css?v=engine-2',
  '/sleepsync-override.css?v=engine-2',
  '/sleepsync-stability.css?v=engine-2',
  '/sleepsync-polish.css?v=130',
  '/sleepsync-notice.css?v=130',
  '/sleepsync-polish.js?v=130',
  '/sleepsync-hydration-v529.js?v=131',
  ...
  '/sleepmate-aurora.css?v=5.3.3',
  '/sleepmate-v530.css?v=5.3.3',
  '/sleepmate-v530.js?v=5.3.3',
  '/o2ring.css?v=5.3.3',
  '/o2ring.js?v=5.3.3',
  '/o2ring-report-ui.js?v=5.3.3',
  '/o2ring-v532.css?v=5.3.3',
  '/o2ring-v532.js?v=5.3.3',
  '/frontend-v533.css?v=5.3.3',
  '/frontend-v533.js?v=5.3.3',
...
const CODE_ASSETS=new Set([
  ...
  '/sleepsync-override.css','/sleepsync-stability.css','/sleepsync-polish.css','/sleepsync-notice.css',
  '/sleepsync-polish.js','/sleepsync-hydration-v529.js','/sleepsync-mobile-v5213.css','/sleepmate-sleep.js',
  '/sleepmate-sleep-v523.js','/sleepmate-chart-v523.js','/sleepmate-sleep-v524.js','/sleepmate-sleep-refresh-v5212.js',
  '/sleepmate-aurora.css','/sleepmate-v530.css','/sleepmate-v530.js','/o2ring.css','/o2ring.js','/o2ring-report-ui.js',
  '/o2ring-v532.css','/o2ring-v532.js','/frontend-v533.css','/frontend-v533.js','/manifest.webmanifest'
]);
...
async function cacheShellAsset(cache,url){
  ...
  try{const response=await fetch(url,{cache:'no-store',signal:controller?.signal});if(response.ok)await cache.put(url,response.clone())}catch{}finally{if(timer)clearTimeout(timer)}
}
...
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  const keys=await caches.keys(),stale=keys.filter(k=>![SHELL_CACHE,API_CACHE].includes(k));
  await Promise.all(stale.map(k=>caches.delete(k)));await self.clients.claim();
  if(stale.length){
...
async function boundedFetch(req,timeout=5000){
  const controller=typeof AbortController==='function'?new AbortController():null,timer=controller?setTimeout(()=>controller.abort(),timeout):null;
  try{return await fetch(req,{cache:'no-store',signal:controller?.signal})}finally{if(timer)clearTimeout(timer)}
}
...
async function offlineClone(r){const h=new Headers(r.headers);h.set('X-SleepMate-Offline','1');return new Response(await r.blob(),{status:200,statusText:'OK (offline cache)',headers:h})}
async function apiNetworkWithFallback(req){
  const cache=await caches.open(API_CACHE),key=apiCacheKey(req);
  try{const fresh=await boundedFetch(req,5000);if(fresh.ok){await cache.put(key,fresh.clone());return fresh}if(backendUnavailable(fresh)){const hit=await cache.match(key);if(hit)return offlineClone(hit)}return fresh}
  catch{const hit=await cache.match(key);if(hit)return offlineClone(hit);throw new Error('offline')}
}
...
async function codeNetworkFirst(req){
  const cache=await caches.open(SHELL_CACHE);
  try{const fresh=await boundedFetch(req,5000);if(fresh.ok){await cache.put(req,fresh.clone());return fresh}if(backendUnavailable(fresh)){const hit=await cache.match(req);if(hit)return hit}return fresh}
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.startsWith('/api/')){if(OFFLINE_API.test(url.pathname)){event.respondWith(apiNetworkWithFallback(req));return}event.respondWith(fetch(req,{cache:'no-store'}));return}
  if(req.mode==='navigate'){event.respondWith(navigationFallback(req));return}
  if(CODE_ASSETS.has(url.pathname)){event.respondWith(codeNetworkFirst(req));return}
  event.respondWith(caches.open(SHELL_CACHE).then(cache=>cache.match(req).then(hit=>hit||fetch(req).then(r=>{if(r.ok&&['image','font'].includes(req.destination))cache.put(req,r.clone());return r}))));
});
